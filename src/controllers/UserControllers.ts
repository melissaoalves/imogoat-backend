  ...
  async signUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!email || !password) {
        res
          .status(400)
          .json({ message: "Email e password são campos obrigatórios" });
        return;
      }
  ...
  async resetPassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!user) {
        res
          .status(400)
          .json({ message: "Código de recuperação inválido ou expirado" });
        return;
      }

      const hashedPassword = await bcrypt.hash(String(newPassword), 10);

      await userRepository.updatePasswordAndClearResetCode(
        user.id,
        hashedPassword
      );
